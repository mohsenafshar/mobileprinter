        ...
        try {
            ...
            while (true) {
                if (waitingForHeader) {
                    ...
                    if (headerIndex == 22) {
                        ...
                        } else if((headerBytes[0] == Constants.HEADER_MSB) && (headerBytes[1] == Constants.HEADER_MMB)) {
                            Log.d(TAG, "run: RECEIVING TEXT");
                            isImage = false;
                            Log.v(TAG, "Header Received.  Now obtaining length");
                            byte[] dataSizeBuffer = Arrays.copyOfRange(headerBytes, 2, 6);
                            progressData.totalSize = Utils.byteArrayToInt(dataSizeBuffer);
                    ...
                    if(!isImage){
                        handler.obtainMessage(MessageType.DATA_TEXT_RECEIVED, bytesRead, -1, buffer).sendToTarget();
                    }
            ...
            if (Utils.digestMatch(data, digest)) {
                if(isImage) {
                    ...
                    Log.v(TAG, "Digest matches OK.It is text");
                    /*Message message = new Message();
                    message.obj = data;
                    message.what = MessageType.DATA_TEXT_RECEIVED;
                    handler.sendMessage(message);*/
